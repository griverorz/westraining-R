...
group means. We used a specific function, `ave` that did precisely that. But the
operation is actually a lot more general. We can think about spliting data
structures by gropus, performing operations in each group and then grouping
...
`tidyverse` approach to `R`. The `tidyverse` is not something we can cover, but
it is probably useful to see a simple example. For instance, consider the

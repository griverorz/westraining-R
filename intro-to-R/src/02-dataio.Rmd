...
other folder/directory by passing a path.
...
The most common format for R uses the extension `.RData`, using the functions
`save` and `load`.
...
foreign formats are provided in the `foreign` package that comes with R. Take a
look at
[the documentation](https://cran.r-project.org/web/packages/foreign/index.html).
We will see how to use additional packages soon enough. 

...
Let's start using the interpreter and get used to the basic arithmetic operations. They all work the way you expect to. 
...
R offers a wide variety of data types. The most relevant ones are: _numeric_, _integer_, _character_, and _logical_.
...
There are good reasons for these "odd" behavior: `NA` is not an assigned value. We have special functions to check whether an object is `NA`:
...
Note that `is.numeric` is a _function,_ and all functions work the same way in R. They have a name followed by the argument(s) enclosed in parenthesis: `any_function(x)`

### Data structures


The most common (and intuitive) data structures are `vector`, `matrix`, `data.frame` and `list`.
...
A matrix is two dimensional, so we need to elements:
```{r eval=FALSE}
